...
//    	String Pass="A1b2c3qwert";
    	...
    	String alreadyVoted = "//*[contains(.,'Вы уже голосовали за это фото')][contains(@class,'b-photo__marks-error_marked font-additional')]" ;
...
//	       		month = (int)Math.floor(1+(Math.random()*12 +1));   //1 - min 12 - max
	       		month = (int)Math.floor((Math.random()*11 +1));   // - min 11 - max //for some reason the url increments the month by 1
	       		day = (int)Math.floor(1+(Math.random()*31 +1));  //1 - min 30 - max
	       		ageRange = (int)Math.floor(Math.random() * ((16-15)+1) + 15);  //15 - min 16 - max
	       		zodiac = "https://my.mail.ru/my/search_people?&gender=2&agerange="+ageRange+"&family=5&birthdayday="+day+"&birthdaymonth="+month+"&infofields=true";
	       		smallLoppSize=50;
...
//ctrl+7
//$x("//*[@title='Вика Ратынская'][@href='/mail/goacora/']")
//$x("//*[@title='Вика Ратынская'][contains(@href, '/mail/goacora/')]")
...
//$x("//*[contains(.,'Вы уже голосовали за это фото')][contains(@class,'b-photo__marks-error_marked font-additional')]") // already voted object
